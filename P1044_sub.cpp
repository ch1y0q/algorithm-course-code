...
struct Edge
{
	Edge(int _u, int _v, double _w, double _t, double _alpha, int _index, Edge* _next) :
		u(_u), v(_v), weight(_w), t_0(_t), alpha(_alpha), index(_index), next(_next) {}
...
inline bool order_cmp(int x, int y) {
	return (O[x] < O[y]);
}

/*
inline bool w_cmp(int x, int y) {
	return (tot_cost[O[x]] > tot_cost[O[y]]);
}
*/
...
int main()
{
	...
	while (T--)
	{
		for (auto ptr : edges) { if (ptr != nullptr)delete ptr; }
		edges.clear();
		...
		rep(_, 1, MAX_ITER) {
			rep(i, 0, n - 1) {
				...
				for (auto it = path[order[i]].begin() + 1; it < path[order[i]].end(); it += 2) {
					int edge_no = *it;
					edges[edge_no]->weight -= edges[edge_no]->t_0 * edges[edge_no]->alpha;
				}
				path[order[i]].clear();
				Dijkstra(O[order[i]], D[order[i]], order[i], adj);
			}
